...
def div_f(var):
    ...
    f_slice = var.get_slice_2D()._slice_2D
    lat = var._geometry.geolat[f_slice]
...
def get_advx(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1, pym6.Dataset(
        ...
        advx = ((-urx).compute() * uh).compute()
        advx = div_f(advx).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    advx.name = 'Zonal advection'
    advx.math = r'$-f^{-1}\bar{\sigma}\hat{u}\hat{u}_{\tilde{x}}$'
...
def get_advy(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1, pym6.Dataset(
        ...
        advy = ((-hvm).compute() * ury).compute()
        advy = div_f(advy).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    advy.name = 'Meridional advection'
    advy.math = r'$-f^{-1}\bar{\sigma}\hat{v}\hat{u}_{\tilde{y}}$'
...
def get_advb(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1, pym6.Dataset(
        ...
        advb = ((-hwm).compute() * urb).compute()
        advb = div_f(advb).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    advb.name = 'Vertical advection'
    advb.math = r'$-f^{-1}\bar{\sigma}\hat{\varpi}\hat{u}_{\tilde{b}}$'
...
def get_cor(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1:
        ...
        cor = hfvm.multiply_by('dxCu').reduce_(np.nansum, axis=3).compute()
...
def get_pfum(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil2, **initializer) as ds2:
        ...
        pfum = div_f(pfum).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    pfum.name = 'Grad of Montg Pot'
    pfum.math = r'$f^{-1}\bar{\sigma}\bar{m}_{\tilde{x}}$'
    pfum.units = r'm$^3$s$^{-2}$'
    return pfum
...
def get_xdivep1(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1:
        ...
        xdivep1 = (-huuxm).compute()
        xdivep1 = div_f(xdivep1).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    return xdivep1
...
def get_xdivep3(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil2, **initializer) as ds2:
        ...
        xdivep3 = (humx * ur).compute()
        xdivep3 = div_f(xdivep3).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    return xdivep3
...
def get_edlsqmx(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1, pym6.Dataset(
        ...
        edlsqmx = div_f(edlsqmx).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    return edlsqmx
...
def get_ydivep1(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1:
        ...
        ydivep1 = huvym.compute()
        ydivep1 = div_f(ydivep1).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    return ydivep1
...
def get_ydivep3(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil2, **initializer) as ds2:
        ...
        ydivep3 = (hvmy * ur).compute()
        ydivep3 = div_f(ydivep3).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    return ydivep3
...
def get_ydivep(fil1, fil2, **initializer):
    ...
    ydivep.math = (r"""-$f^{-1}(\bar{\sigma}\widehat{u ^{\prime \prime} """
...
def get_bdivep1(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1:
        ...
        bdivep1 = div_f(huwbm).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    return bdivep1
...
def get_bdivep3(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil2, **initializer) as ds2:
        ...
        bdivep3 = (hwb * ur).compute()
        bdivep3 = div_f(bdivep3).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
    return bdivep3
...
def get_bdivep4(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1, pym6.Dataset(
        ...
        pfum = ds2.PFu.read().nanmean(axis=0).compute()
        db = np.diff(ds1.zl)[0] * 9.8 / 1000
        edpfudmb = -((-hpfu).compute() + (h * pfum).compute()).compute()
        edpfudmb = div_f(edpfudmb).multiply_by('dxCu').reduce_(
            np.nansum, axis=3).compute()
        bdivep4 = (edpfudmb + (edlsqmx * db * 0.5).compute()).compute()
    bdivep4.name = 'Form drag'
    bdivep4.math = (
        r"""$f^{-1}(\overline{\zeta ^\prime m_{\tilde{x}}^\prime})"""
        r"""_{\tilde{b}}$""")
    bdivep4.units = r'm$^3$s$^{-2}$'
    return bdivep4
...
def get_X1twa(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1:
        ...
        X1twa = (hdiffum.multiply_by('dxCu').reduce_(np.nansum,
                                                     axis=3)).compute()
        X1twa.name = 'Horizontal friction'
        X1twa.math = r'$f^{-1}\bar{\sigma}\widehat{X^H}$'
...
def get_X2twa(fil1, fil2, **initializer):
    ...
    with pym6.Dataset(fil1, **initializer) as ds1:
        ...
        X2twa = (hdudtviscm.multiply_by('dxCu').reduce_(np.nansum,
                                                        axis=3)).compute()
        X2twa.name = 'Vertical viscous forces'
        X2twa.math = r'$f^{-1}\bar{\sigma}\widehat{X^V}$'
...
def extract_twamomx_terms(fil1, fil2, **initializer):
    ...
    conventional_list = [
        get_advx, get_advy, get_advb, get_pfum, get_xdivRS, get_xdivep4,
